...
public class HashTableOpenAddressing<K, V> implements DictionaryInterface<K, V> {
	...
	public V add(K keyIn, V valueIn) {
		...
		if ( (table[index] == null) || (table[index].isRemoved() ) ) {
			TableEntry<K, V> newEntry = new TableEntry<>(keyIn, valueIn);
		...
		while (!found && table[index] != null) {	
			if (table[index].isIn()) {
				if (key.equals(table[index].getKey()))
					...
					index = (int) ((originalIndex + Math.pow(i, 2)) % table.length);       
			}
			else { 						// Skip entries that were removed
				// Save index of first location in removed state
				if (removedStateIndex == -1)
...
//		index = quadraticProbe(index, key);
	...
	public boolean isFull() {
		if ( (getSize() / table.length) > loadFactor ) {
			return true;
		}
	...
	private void checkInitialization()  {
		if (!initialized)
			throw new SecurityException("HashedDictionary object not initialized.");
	} 
